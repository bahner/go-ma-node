...
func (s *Subscription) Init(sp *gen.ServerProcess, args ...gen.MessageEvent) error {
	...
	sp.Process.Send(s.owner, gen.Tuple{
		gen.Atom(":go_space_topic_subscription_created"),
		gen.String(s.actor.Entity.Topic.String()),
	})

	return nil
}

func (s *Subscription) HandleCast(server_procces *gen.ServerProcess, message gen.MessageEvent) gen.ServerStatus {
	log.Debugf("Received message: %s", message)
	return gen.ServerStatusOK
}

func (s *Subscription) HandleCall(serverProcess *gen.ServerProcess, from gen.ServerFrom, message gen.MessageEvent) (gen.MessageEvent, gen.ServerStatus) {
	...
	if err != nil {
		...
		return gen.Atom("error"), gen.ServerStatusOK
	}
	...
	case "publish":
		...
		return gen.Atom("ok"), gen.ServerStatusOK
...
func (s *Subscription) HandleInfo(serverProcess *gen.ServerProcess, message gen.MessageEvent) gen.ServerStatus {
	log.Debugf("Received message: %s", message)
	return gen.ServerStatusOK
}
...
func extractActionData(term gen.MessageEvent) (gen.Atom, []gen.MessageEvent, error) {
	// If the term is just an Atom
	if atom, ok := term.(gen.Atom); ok {
		return atom, nil, nil
	}

	// If the term is a Tuple
	tuple, ok := term.(gen.Tuple)
	if !ok || len(tuple) == 0 {
		return "", nil, errors.New("term is not a tuple or is empty")
	}

	command, ok := tuple[0].(gen.Atom)
	if !ok {
